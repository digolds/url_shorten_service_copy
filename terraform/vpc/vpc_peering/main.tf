...
resource "aws_route" "route_to_vpc_app" {
  route_table_id            = data.terraform_remote_state.vpc_codebuild_state.outputs.vpc_instance.main_route_table_id
  destination_cidr_block    = data.terraform_remote_state.vpc_app_state.outputs.vpc_instance.cidr_block
  vpc_peering_connection_id = aws_vpc_peering_connection.vpc_peering.id
}

resource "aws_route" "route_to_vpc_codebuild" {
  route_table_id            = data.terraform_remote_state.vpc_app_state.outputs.vpc_instance.main_route_table_id
...
locals {
  ...
  vpc_app_name      = data.terraform_remote_state.vpc_app_state.outputs.vpc_instance.tags.Name
...
resource "aws_security_group" "sg_for_cache" {
  ...
  vpc_id      = data.terraform_remote_state.vpc_app_state.outputs.vpc_instance.id
  ...
  ingress {
    ...
    protocol    = "tcp"
    cidr_blocks = [data.terraform_remote_state.vpc_app_state.outputs.vpc_instance.cidr_block,
...
resource "aws_security_group" "sg_for_vpc_app" {
  name        = "${local.vpc_name}-attach-to-codebuild"
